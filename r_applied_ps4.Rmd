...
# separate latitude and longitude from the geo column
dfw <- separate(dfw, geoWKT, into = c("latitude", "longitude"), sep = ",", convert = TRUE)
...
# Convert alert_date to POSIXct object with unknown time zone
dfw$alert_date <- ymd_hms(dfw$ts)

# Convert alert_date to Central Time
dfw_alert_date_ct <- with_tz(dfw$ts, "America/Chicago")

# Round alert_date_ct to nearest 5 minutes
dfw_alert_date_ct_rounded <- round_date(dfw_alert_date_ct, "5 minutes")
```
...
# subset data for N Western Ave accidents
corridor_accidents <- dfw[dfw$street == "N Western Ave" & dfw$type == "ACCIDENT",]

# get map centered on the accidents
map <- get_stamenmap(
  bbox = c(
    left = min(corridor_accidents$longitude),
...
# plot the map and accidents
ggmap(map) +
  geom_point(
    data = corridor_accidents,
  ...
  labs(
    title = "Accidents on High Crash Corridor # 7",
    subtitle = "N Western Ave",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
  theme(
    ...
    plot.subtitle = element_text(size = rel(1.2), face = "bold", hjust = 0.5)
  )
